...
function renderGrid(list){
  ...
  if(!g) return;
  document.getElementById('countLabel').textContent = list.length + ' ' + T('f.count');
  if(list.length===0){
    g.innerHTML = '<div style="grid-column:1/-1;background:#121212;border:1px dashed #2a2a2a;border-radius:16px;padding:32px;text-align:center;color:#777">' + T('f.empty') + '</div>';
  } else {
    g.innerHTML = list.map(op=>{ const p = (typeof prodLoc==='function') ? prodLoc(op) : op; return `
      <article class="card">
        <div class="card-top">
        ...
        <div class="card-body">
          ...
          <div class="actions">
            ...
            <button class="btn-get" onclick="buyNow(${p.id})">${T('f.get')}</button>
          </div>
        </div>
      </article>
    `;}).join('');
...
function addToCart(id){
  ...
  saveCart(); updateCart(); toast(T('cart.added')+p.name);
}
...
function updateCart(){
  ...
  if(!wrap) return;
  if(cart.length===0){
    wrap.innerHTML='<div class="empty">'+T('cart.empty')+'</div>';
  } else {
...
function syncCart(){
  try {
    ...
    const clean = arr
      ...
      .filter(Boolean);
    if(clean.length || arr.length === 0) cart = clean;
  } catch(e){}
}
function openCart(){
  try{
...
function checkout(){
  if(cart.length===0) return toast(T('cart.empty2'));
  saveCart();
  window.location.href = 'pages/checkout.html';
}
